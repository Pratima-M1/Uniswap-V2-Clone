...
async function main() {
  ...
  //Here new Test ERC20 tokens are deployed and minted or we can use existing ERC20 tokens
  ...
  // 18. Initialize weth contract address
  ...
  const RouterContract = new Contract(routerAddress, routerArtifact.abi, owner);
  const MaxUint256 =
  ...
  const addLiquidityTx = await RouterContract.connect(owner).addLiquidity(
    usdtAddress,
    usdcAddress,
    token0Amount,
    token1Amount,
